...
final class ContactsViewModelTests: XCTestCase {
    ...
    override func setUp() {
        super.setUp()
        container = Container()
        container.register(ContactsRepository.self, factory: { _ in
    ...
    func test_whenSearch_thenShouldFilterResults() async throws {
        ...
        XCTAssertEqual(viewModel.contacts[ContactsStrings.contactsSection.rawValue]!.map { $0.name }, ["Arnold Watson", "Bernard Edwards"])
    }
    
    func test_whenSearchByPhoneNumber_thenFilterResults() async throws {
        //given
        let query = "0204"
        try await viewModel.fetch()
        ...
        XCTAssertEqual(viewModel.contacts[ContactsStrings.contactsSection.rawValue]!.map { $0.phoneNumber }, ["+1 202-555-0204"])
    }
